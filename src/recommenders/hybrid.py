...
# recommender scores function for fixed-weights hybrid model
# input: recommendations produced by each score, fixed weights (automatically less for content due to poor metadata) / output: dataframe with columns=resource id and all scores + hybrid score
def hybrid_scores(
    popularity_scores,
    content_scores,
    ...
    hybrid_data = popularity_data.merge(
        content_data,
        on=["id_site", "code_module", "code_presentation", "activity_type"],
        how="outer",
    )
    ...
    hybrid_data[["popularity_score", "content_score", "collaborative_score"]] = (
        hybrid_data[
    ...
    hybrid_data["hybrid_score"] = (
        (popularity_weight * hybrid_data["popularity_score"])
        + (content_weight * hybrid_data["content_score"])
        + (collaborative_weight * hybrid_data["collaborative_score"])
    )
    ...
    # return new dataframe of fixed weight hybrid model recommendations
    return hybrid_data
