# standard imports for data storage
import pandas as pd


# creates data to be used in creation of ML features without data leakage
# input: datasets object with all processed dataframes, cutoff day to filter data / output: datasets object with filtered preprocessed data according to cutoff day
def filter_data_by_cutoff(datasets, cutoff_day):
    ...
    cutoff_datasets["assessment_data"] = cutoff_datasets["assessment_data"][
        cutoff_datasets["assessment_data"]["date_submitted"] <= cutoff_day
    ].copy()

    # 2. for vle data, cutoff by date
    cutoff_datasets["vle_data"] = cutoff_datasets["vle_data"][
        cutoff_datasets["vle_data"]["date"] <= cutoff_day
    ].copy()

    # 3. for student data, cutoff by registration date
    cutoff_datasets["student_data"] = cutoff_datasets["student_data"][
        cutoff_datasets["student_data"]["date_registration"] <= cutoff_day
    ].copy()
    ...
    cutoff_datasets["student_data"] = cutoff_datasets["student_data"].drop(
        columns=[
    ...
    cutoff_datasets["assessment_data"] = cutoff_datasets["assessment_data"].drop(
...
# create ML assessment features
# input: assessment data preprocessed dataframe / output: assessment features dataframe for each student
def create_assessment_features(assessment_data):
    ...
    assessment_features = (
        ...
        .agg(
            ...
            average_submission_date=("date_submitted", "mean"),
            average_assessment_weight=("weight", "mean"),
            assessment_type_diversity=("assessment_type", "nunique"),
        )
        .reset_index()
    )
    ...
    # return grouped + aggregated features dataframe
    return assessment_features


# create ML vle features
# input: vle data preprocessed dataframe / output: vle features dataframe for each student
def create_vle_features(vle_data):
    ...
    # new columnName =  (column from assessment data used in aggregation, aggregation function)
    vle_features = (
        vle_data.groupby(["id_student", "code_module", "code_presentation"])
        .agg(
            total_resource_clicks=("sum_click", "sum"),
            average_resource_clicks=("sum_click", "mean"),
            first_used=("date", "min"),
            last_used=("date", "max"),
    ...
    # separate features, calculated from 2 differnt columns (maximize usefulness)
    vle_features["interaction_duration"] = (
        vle_features["last_used"] - vle_features["first_used"]
    )
...
# create ALL features combined with student features
# input: datasets object with all preprocessed datafreames, cutoff day to create filtered features / output: single student features dataframe for each student
def create_features(datasets, cutoff_day=86):
    ...
    assessment_features = create_assessment_features(
        historical_datasets["assessment_data"]
    )
    ...
    student_features = historical_datasets["student_data"].merge(
        assessment_features,
    ...
    student_features["days_registered"] = (
        cutoff_day - student_features["date_registration"]
    )
