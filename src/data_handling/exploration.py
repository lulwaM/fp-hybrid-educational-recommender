#imports for data storage and data visualizations
import pandas as pd
import matplotlib.pyplot as plt

def explore_student_data(student_data):
    #save all plots in relevant images folder
...
def explore_resource_data(resource_data):
    ...
    vle_crosstab = pd.crosstab(resource_data["code_module"], resource_data["activity_type"], normalize="index")
...
def explore_interaction_data(interaction_data):
    ...
    #code inspiration from:https://www.kaggle.com/code/ayoubaziba/studentperformancenalysis
    interaction_data.groupby("id_student")["total_resource_clicks"].sum().hist(color='lightgreen')
    #end inspired code
    ...
    interaction_data.groupby("activity_type")["click_duration"].mean().sort_values().plot(kind='barh',color='violet')
