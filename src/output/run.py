...
if __name__ == '__main__':
    ...
    if bool_save_sample_recommendations:
        ...
        recommendations = pipeline.generate_recommendations(id_student=6516,code_module='AAA',code_presentation='2014J',datasets=processed_datasets,model=model,cutoff_day=86,k=20)
    ...
    # conditionals that call functions according to boolean values

    if bool_baseline_single:
        pipeline.run_baseline_evaluation_single(processed_datasets=processed_datasets,cutoff_day=86,evaluated_student=6516,k=20)

    if bool_baseline_holdout:
        (baseline_holdout_results, baseline_holdout_summary) = pipeline.run_baseline_evaluation_holdout(processed_datasets=processed_datasets,cutoff_day=150,k=20)

    if bool_baseline_cross_validation:
        (baseline_cv_results, baseline_cv_averages) = pipeline.run_baseline_cross_validation(processed_datasets=processed_datasets,cutoff_days=[60,120,180],test_size=60,k=20)

    if bool_prepare_ML_data:
        training_dataset, test_dataset = pipeline.run_prepare_ML_data(processed_datasets=processed_datasets,training_cutoff=86,test_cutoff=150,k=20)
    ...
    if bool_train_sampled_random_forest:
        random_forest_sample_model = pipeline.run_train_ML(model_type='random_forest',sample_size=10000)
    ...
    if bool_evaluate_ML_recommender_random_forest:
        (random_forest_results,random_forest_summary) = pipeline.run_evaluate_ML_recommender(processed_datasets=processed_datasets,file_name='random_forest_recommender_results',model_type='random_forest',model=None,model_path='models/random_forest_model.joblib',test_cutoff=150,k=20,saveData=True,saveSummary=True)

    random_forest_sample_summary = None
    if bool_evaluate_ML_recommender_sampled_random_forest:
        (random_forest_sample_results,random_forest_sample_summary) = pipeline.run_evaluate_ML_recommender(processed_datasets=processed_datasets,file_name='random_forest_sample_recommender_results',model_type='random_forest',model=None,model_path='models/random_forest_sample_model.joblib',test_cutoff=150,k=20,saveSummary=False)

    SVC_sample_summary = None
    if bool_evaluate_ML_recommender_sampled_SVC:
        (SVC_sample_results,SVC_sample_summary) = pipeline.run_evaluate_ML_recommender(processed_datasets=processed_datasets,file_name='SVC_sample_recommender_results',model_type='SVC',model=None,model_path='models/SVC_sample_model.joblib',test_cutoff=150,k=20,saveSummary=False)

    if random_forest_sample_summary is not None and SVC_sample_summary is not None:
        pipeline.run_save_ML_comparison_results(random_forest_sample_summary,SVC_sample_summary)

    if bool_evaluate_ML_classifier_random_forest:
        random_forest_classifier_results = pipeline.run_evaluate_ML_classifier(model=None,model_type='random_forest',model_path='models/random_forest_model.joblib',file_name='random_forest_classifier_results')

    if bool_evaluate_ML_classifier_sampled_random_forest:
        random_forest_sample_classifier_results = pipeline.run_evaluate_ML_classifier(model=None,model_type='random_forest',model_path='models/random_forest_sample_model.joblib',file_name='random_forest_sample_classifier_results')

    if bool_evaluate_ML_classifier_sampled_SVC:
        SVC_sample_classifier_results = pipeline.run_evaluate_ML_classifier(model=None,model_type='SVC',model_path='models/SVC_sample_model.joblib',file_name='SVC_sample_classifier_results')

    if bool_evaluate_ML_cross_validation_random_forest:
        (random_forest_cv_results, random_forest_cv_summary) = pipeline.run_evaluate_ML_cross_validation(processed_datasets=processed_datasets,cutoff_days=[60,120,180],test_size=60,k=20,model_type='random_forest')

    if bool_generate_API_recommendations:
        model = ranking.load_model('models/random_forest_model.joblib')
        api_datasets = preprocessing.load_api_datasets()
        print(pipeline.generate_API_recommendations(id_student=6516,code_module='AAA',code_presentation='2014J',datasets=api_datasets,model=model))

    if bool_generate_recommendations:
        model = ranking.load_model('models/random_forest_model.joblib')
        print(pipeline.generate_recommendations(id_student=6516,code_module='AAA',code_presentation='2014J',datasets=processed_datasets,model=model,cutoff_day=86,k=20))
