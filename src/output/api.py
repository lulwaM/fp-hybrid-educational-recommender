...
# setting key identifiers
app = FastAPI(title="Educational Content Recommender API", description="Returns personalized educational recommendations based on id_student, code_module, code_presentation inputs")
...
@app.get("/",summary='Entry page', description='Provides instructions on accessing app routes to make a request')
async def root():
    return {"message": "Educational content recommender API is running! Please visit the /docs page to view routes and make a request."}
# end copied and adapted code

# to get recs, use GET HTTP request (as per conventions)
...
@app.get('/recommendations/{id_student}', summary='Generate student recommendations', description='Returns the top 20 educational resource recommendations for a specified student id, code module, and code presentation, using 86 as the cutoff day')
    # must define type
